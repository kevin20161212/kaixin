...
$(document).ready(function(){
   ...
   /* $(".layer-dialog-2").on("click",'.layer-dialog-2',function(){
  	    var mess= $(this).attr("data-mess");
  	    var successCall= $(this).attr("data-succ");
  	    var httpUrl=$(this).attr("data-url");
  	    var params= $(this).attr("data-params");
     ...
     * post    formdata
     */
    $(document).on('click',".ajax-post2",function(){
    	
  	    var successCall= $(this).attr("data-succ");
  	    var httpUrl=$(this).attr("data-url");
  	    var params= $(this).attr("data-params");
  	    gl.sendAjax(httpUrl,params,successCall);
    ...
    $(document).on("click",".ajax-post",function(){
      ...
      formObj.children("input").each(function(){
       	  var inputMess=$(this).data("mess")
       	  if($(this).hasClass("tel"))//手机号格式检测
       	  {
       	  	if(this.value==""){proContinue=false; gl.layer_dialog_1("请输入"+inputMess);this.focus();return false;}
       	  	if(!gl.telCheck(this.value)){proContinue=false; gl.layer_dialog_1(inputMess+"格式不正确");this.focus();return false;}
       	  }
       	  if($(this).hasClass("money"))//金额格式检测
       	  {
       	  	if(this.value==""){proContinue=false; gl.layer_dialog_1("请输入"+inputMess);this.focus();return false;}
       	  	if(!gl.moneyCheck(this.value)){proContinue=false; gl.layer_dialog_1(inputMess+"格式不正确");this.focus();return false;}
       	  }
       	  if($(this).hasClass("int"))//整数格式检测
       	  {
       	  	if(this.value==""){proContinue=false; gl.layer_dialog_1("请输入"+inputMess);this.focus();return false;}
       	  	if(!gl.intCheck(this.value)){proContinue=false; gl.layer_dialog_1(inputMess+"格式不正确");this.focus();return false;}
       	  }
       ...
       formObj.children("textarea").each(function(){
       	  if($(this).hasClass("notEmpty"))//是否为空检测
       	  {
       	  	var inputMess=$(this).data("mess")
       	  	if(this.value==""){proContinue=false; gl.layer_dialog_1("请输入"+inputMess);this.focus();return false;}
       ...
       formObj.children("select").each(function(){
       	  if($(this).hasClass("notEmpty"))//是否为空检测
       	  {
       	  	var inputMess=$(this).data("mess")
       	  	if(this.value==""){proContinue=false; gl.layer_dialog_1("请选择"+inputMess);this.focus();return false;}
       	  }
       })
       if(!proContinue){return false} //被检测信息有不合法情况，直接退出
       
    	if(formObj.children("input[type='file']").size()>0)
    	{
    	    query=new FormData(formObj.get(0));
    	    if($(that).hasClass("confirm")){gl.layer_dialog_2(succ,"确定要提交信息吗?",action,query,true)}else{gl.showaddWait2();l.sendFormData(action,query,succ)}
    	}
    	else
    	{
    		query=gl.jQseri(formObj.get(0));
    		this.Debug ? alert(query) :"";
    		if($(that).hasClass("confirm")){gl.layer_dialog_2(succ,"确定要提交信息吗?",action,query,false)}else{gl.showaddWait2();gl.sendAjax(action,query,succ)}
    	}
    })
    /*
     * 移动端头部导航
		...
		*/
		$(".city-picker").on("click",function(){
	        ...
	        cityPicker3.show(function(items) {
	        	...
	        	{
	        		size ==2 ? showDom.value = items[0].text+"-"+items[1].text : showDom.value = items[0].text+"-"+items[1].text+"-"+items[2].text
	        	}
	        	else
	        	{
	        		size ==2 ? showDom.innerHTML = items[0].text+"-"+items[1].text : showDom.innerHTML = items[0].text+"-"+items[1].text+"-"+items[2].text
	        	}
	        })
		})
		/*
...
GLJS.prototype.layer_dialog_2=function(succ,message,httpUrl,params,isFd){
		if(httpUrl)
		{
			...
			{
				if(isFd){gl.sendFormData(httpUrl,params,succ)}else{gl.sendAjax(httpUrl,params,succ)}
			});
		}
...
GLJS.prototype.sendAjax=function(httpUrl,params,callBack){
	    //alert("POST发送序列化数据"+params)
...
GLJS.prototype.sendFormData=function(httpUrl,fd,callBack){
	var that=this;
		...
		$.ajax({
			...
			},error:function(){that.layer_dialog_1("FormData发送网络异常")},
		})
	},
/*
*/
 ...
 *        
 */
GLJS.prototype.muiOpenPage=function(/*mmui,url,nocache,position,params,show*/){
 ...
 * 
    ...
    var docEl = doc.documentElement,
        ...
        recalc = function () {
            ...
            //可以根据实际需要修改
            clientWidth>1000?clientWidth=1000:clientWidth=clientWidth;

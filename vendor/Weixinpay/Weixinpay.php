...
class Weixinpay {
    ...
    public function __construct(){
        ...
        $config = M('wechat_config')->find();
      
        $this->config=array(
            'APPID'              => $config['appid'], // 微信支付APPID
            'MCHID'              => $config['mch_id'], // 微信支付MCHID 商户收款账号
            'KEY'                => $config['skey'], // 微信支付KEY
            'APPSECRET'          => $config['appsecret'],  //公众帐号secert
            'TOKEN'              => $config['token'],
            'NOTIFY_URL'         => 'http://nuoche.youfai.cn/index.php/home/notify/notify/', // 接收支付状态的连接
        );
    }
    ...
    **/
    ...
    public function getParameters($order){
        ...
        // 如果没有get参数没有code；则重定向去获取openid；
			...
			// dump(session('wxuser')['open_id']);exit;
            // 统一下单 获取prepay_id

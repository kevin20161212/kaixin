...
class ListBuilder extends Controller
{
    ...
    {
         if(input('modal')){
               $this->_template = BUILDER_DIR . 'layout/' . strtolower(MODULE_MARK) . '/model.html';
            }else{
     ...
     * @author <youfai@youfai.cn>
     */
    public function setTopAlert($top_alert)
    {
        $this->_top_alert = $top_alert;
        return $this;
    }
     ...
     * @author <youfai@youfai.cn>
     */
    public function search_form_items($name, $type, $title, $tip, $options = array(), $extra = '', $extra_attr = '')
    {
         $item['name']           = $name;
        $item['type']           = $type;
        $item['title']          = $title;
     ...
     * @author <youfai@youfai.cn>
     */
    public function addSearchItem($name, $type, $title, $tip, $options = array(), $extra = '', $extra_attr = '')
    {
         $item['name']           = $name;
        $item['type']           = $type;
        $item['title']          = $title;
        ...
        if (is_array($extra)) {
            ...
            $item['extra']['class'] = $extra?:'col-lg-4 col-md-3';
        }
        $item['extra']['attr'] = $extra_attr;
        $this->_search_form_items[]   = $item;
        return $this;
    ...
    public function addRightButton($type, $attribute = null)
    {
        ...
        switch ($type) {
            ...
            case 'editmodal': // 编辑按钮
                ...
                $my_attribute['href']  = U(
                    request()->module() . '/' . request()->controller() . '/edit',
                    array($this->_table_data_list_key => '__data_id__','modal'=>true)
                );
                $my_attribute['data-toggle']  = 'modal';
                $my_attribute['data-target']  = '#myModal';
                $my_attribute['data-keyboard']  = 'false';
            ...
            case 'forbid': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                ...
                $my_attribute['forbid0']['title'] = $attribute['title0']?:'启用';
                ...
                $my_attribute['forbid1']['title'] = $attribute['title1']?:'禁用';
    ...
    public function display($template = '', $vars = [], $replace = [], $config = [])
    {
        ...
        foreach ($this->_table_data_list as $key => &$data) {
            ...
            // dump($this->_right_button_list);exit;
                    ...
                    case 'status':
                        switch ($data[$column['name']]) {
                            case '-1':
                                ...
                                break;
                            case '0':
                                $data[$column['name']] = '<i  style="color:red;"  class="fa fa-ban text-danger"></i>';
                                break;
                            case '1':
                                $data[$column['name']] = '<i style="color:#78b335;" class="fa fa-check text-success"></i>';
                                break;
                        }
                        break;
                    case 'zhifu':
                        switch ($data[$column['name']]) {
                            case '1':
                                $data[$column['name']] = '<i class="label label-primary">未支付</i>';
                                break;
                            case '2':
                                $data[$column['name']] = '<i class="label label-success">已支付</i>';
                                break;
                        }
                        break;
                    case 'zhifu_type':
                        switch ($data[$column['name']]) {
                            case '1':
                                $data[$column['name']] = '<i style="background:#00D30C;" class="label label-primary">微信支付</i>';
                                break;
                            case '2':
                                $data[$column['name']] = '<i style="background:#00A5E8;" class="label label-success">支付宝支付</i>';
                                break;
                        }
                        break;
                    case 'sex':
                        switch ($data[$column['name']]) {
                            case '0':
                                ...
                                break;
                            case '1':
                                $data[$column['name']] = '<i  style="color:#25AAE3;font-size:18px;"  class="fa fa-male text-danger"></i>';
                                break;
                            case '2':
                                $data[$column['name']] = '<i style="color:#EA0A62;font-size:18px;" class="fa fa-female text-success"></i>';
                                break;
                        }
                        break;
                    case 'byte':
        ...
        // dump($this->_search_form_items);exit;
        // dump($this->_right_button_list);exit;
        ...
        $this->assign('extra_html', $this->_extra_html); // 额外HTML代码
         $this->assign('top_alert', $this->_top_alert); // 搜索配置
        // dump($this->_table_data_list);exit;
        // 显示页面

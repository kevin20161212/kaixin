...
class Controller extends \think\Controller
{
    ...
    protected function display($template = '', $vars = [], $replace = [], $config = [])
    {
        ...
        // dump(strtolower(request()->module() . '_' . request()->controller() . '_' . request()->action()));exit;
        ...
        // dump(C('LISTBUILDER_LAYOUT'));exit;
        // dump($_admin_public_layout);
        // exit;
...
//dump($template);exit;
        if (request()->isAjax()) {
            ...
            // dump($this->fetch($template));exit;
           // dump(Request::instance()->param('modal'));exit;

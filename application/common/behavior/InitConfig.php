    ...
    public function run(&$content)
    {
        ...
        if (!$system_config || APP_DEBUG === true) {
            ...
            foreach ($module_list as $_module) {
                ...
                $adminConfig = json_decode($_module['config'], true);
                foreach ($adminConfig as $key => $val) {
                    if ($val['type'] == 'group') {

                        foreach ($val['options'] as $key2 => $val2) {
                            foreach ($val2['options'] as $key3 => $val3) {
                                $module_config[strtolower($_module['name'] . '_config')][$key3]  = $val3['value'];
                            }
                        }
                    } else {
                         $module_config[strtolower($_module['name'] . '_config')][$key]  = $val;
                    }
                }
                $roleConfig = json_decode($_module['role_config'], true);
                // dump($roleConfig);die;
                if($roleConfig){
                    foreach ($roleConfig as $keys => $vals) {
                        foreach ($vals as $key => $val) {
                            if ($val['type'] == 'group') {
                                foreach ($val['options'] as $key2 => $val2) {
                                    foreach ($val2['options'] as $key3 => $val3) {
                                        $module_config[strtolower($_module['name'] . '_config')][$keys][$key3]  = $val3['value'];
                                    }
                                }
                            } else {
                                 $module_config[strtolower($_module['name'] . '_config')][$keys][$key]  = $val;
                            }
                        }
                    }
                }
            ...
            // exit;
                ...
                // $module_config[strtolower($_module['name'] . '_config')]                = json_decode($_module['role_config'], true);
                ...
                if (is_file($bhv_path)) {
                    \think\Hook::add('yfthink_behavior', $_module['name'] . '\\behavior\\' . $_module['name'] . 'Behavior');
                }
            }
            if ($module_config) {
                // 合并模块配置
            ...
            foreach ($addon_list as $_addon) {
                ...
                if (is_file($bhv_path)) {
                    \think\Hook::add('yfthink_behavior', '\\addon\\' . lcfirst($_addon['name']) . '\\behavior\\' . $_addon['name'] . 'Addon');
                }
            }
            if ($addon_config) {
                // 合并模块配置

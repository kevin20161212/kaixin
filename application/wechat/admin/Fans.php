...
use app\wechat\stores\controller\BasicAdmin;
use app\wechat\stores\service\LogService;
use app\wechat\stores\service\ToolsService;
...
class Fans extends BasicAdmin
{
    ...
    public function index()
    {
        ...
        foreach (['nickname', 'country', 'province', 'city'] as $key) {
            (isset($get[$key]) && $get[$key] !== '') && $map[$key] = array('like', '%' . $get[$key] . '%');
        }
        if (isset($get['create_at']) && $get['create_at'] !== '') {
            list($start, $end) = explode(' - ', $get['create_at']);
            $map['subscribe_time'] = array('between', array("{$start} 00:00:00", "{$end} 23:59:59"));
        }
       $map['is_black'] = 0;
       (isset($get['sex']) && $get['sex'] !== '') && $map['sex'] = $get['sex'];

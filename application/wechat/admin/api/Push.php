...
namespace app\wechat\admin\api;
        ...
        switch ($info['type']) {
            ...
            case 'news':
                ...
                foreach ($news['articles'] as $vo) {
                    ...
                    $data[] = ['Title'=>$vo['title'],'Description'=>$vo['digest'],'PicUrl' =>'http://minsu.youfai.cn'.$vo['local_url'],'Url' =>$url];
                }
        ...
        switch (strtolower(sysconf('wechat_type'))) {
            case 'api': // 参数对接，直接回复XML来实现消息回复
                ...
                switch (strtolower($type)) {
                    ...
                    case 'news':
                        file_put_contents('/mnt/www/minsu.youfai.cn/application/wechat/admin/api/eie.txt',var_export($wechat->news($data['articles'])->reply([], true),true));

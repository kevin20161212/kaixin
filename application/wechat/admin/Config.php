...
class Config extends BasicAdmin
{
    ...
    public function index()
    {
        ...
        if ($this->request->isGet()) {
            $code = encode(url('@wechat/config/index', '', true, true)) ;
           
            $data = [
                'title'   => '微信接口配置',
                'appid'   => $this->request->get('appid', sysconf('wechat_thr_appid')),
                'appkey'  => $this->request->get('appkey', sysconf('wechat_thr_appkey')),
                'authurl' => "https://service.thinkadmin.top/wechat/api.push/auth/{$code}.html",
            ];
        ...
        $this->success('微信授权数据修改成功！', url('@wechat/config/index', '', true, true));
    }

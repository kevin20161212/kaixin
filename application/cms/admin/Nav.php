...
class Nav extends Admin {
    ...
    public function index($group = 'main') {
       //搜索
        ...
        // 设置Tab导航数据列表
        $nav_group_list = C('NAV_GROUP_LISTS'); // 获取分类分组
        ...
        $builder->setMetaTitle('导航列表') // 设置页面标题
            ...
            ->addTopButton('resume',array('model'=>'cms/cms_nav')) // 添加启用按钮
            ->addTopButton('forbid',array('model'=>'cms/cms_nav')) // 添加禁用按钮
            ->addTopButton('delete',array('model'=>'cms/cms_nav')) // 添加删除按钮
            ...
            ->addRightButton('forbid',array('model'=>'cms/cms_nav')) // 添加禁用/启用按钮
            ->addRightButton('delete',array('model'=>'cms/cms_nav')) // 添加删除按钮
            ->display();
    }
...
EOF;
    public function add($group){
        if (request()->isPost()) {
          ...
          // 获取前台模版供选择
            if (C('CURRENT_THEME')) {
                $template_list = \yfthink\File::get_dirs(getcwd() . '/Theme/' . C('CURRENT_THEME') . '/Home//Nav');
            } else {
                $template_list = \yfthink\File::get_dirs(getcwd() . '/Application/Home/View/Nav');
            }
            ...
            // dump($template_list_detail);die;
            // dump(select_list_as_tree('cms_nav', array('group' => $group), '顶级导航'));exit;
            ...
            $builder->setMetaTitle('新增导航') // 设置页面标题
                ...
                ->addFormItem('pid', 'select', '上级导航', '上级导航', select_list_as_tree('cms_nav', array('group' => $group), '顶级导航'))
                ->addFormItem('title', 'text', '导航标题', '导航前台显示标题')
                ->addFormItem('type', 'radio', '导航类型', '导航类型', D('cms_nav')->nav_type())

...
class Store extends Controller
{
    ...
    protected function _initialize()
    {
       ...
       // dump(model('admin/Group')->checkMenuAuth());die;
        ...
        $this->assign('mokuainames',$mokuainames);
        // 获取左侧导航
    ...
    public function module_config()
    {
        if (request()->isPost()) {
            ...
            $names = MODULE_MARK == 'Admin'?'config':'role_config';
            if($names == 'config'){
                $configs = $config;
            }else{
                $configs[MODULE_MARK] = $config;
            }
            dump($configs);exit;
            $flag   = model('Admin/Module')
                ->where("id={$id}")
                ->setField($names, json_encode($configs));
            if ($flag !== false) {
                $this->success('保存成功');
            } else {
                $this->error('保存失败');
    ...
    private function module_config_get()
    {
            ...
            if ($db_config) {
                if(MODULE_MARK == 'Admin'){
                    ...
                    $db_config = $db_config_en[$markName];
                    unset($db_config_en);
                    unset($module_config['config']);
                }
    ...
    public function module_config_add()
    {
        ...
        if (request()->isPost()) {
            ...
            if($names == 'config'){
                ...
                $config[MODULE_MARK] = $module_config['config'];
            }
            $flag   = model('Admin/Module')
                ->where("id={$id}")
                ->setField($names, json_encode($config));
            if ($flag !== false) {
                $this->success('保存成功');
            } else {
                $this->error('保存失败');

...
class CmsTender extends Model {
     ...
     * @author youfai.cn <280962430@qq.com>
     */
    protected $tableName = 'cms_tender';
    ...
    protected $_validate = array(
        ...
        array('textarea', 'require', '简介不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
    ...
    public function lists($where=[],$type=0,$page=1,$num=8,$type1=false){
        if($type==1){
            ...
            if($type1 == true){
                ...
                $data = $this->order('id desc')->where(['pid'=>['in',$ids],'authen_status'=>1])->limit(($page-1)*$num,$num)->select();
            }else{
                //dump($where);
                $where['authen_status'] = 1;
